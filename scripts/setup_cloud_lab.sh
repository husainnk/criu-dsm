...
ssh -o StrictHostKeyChecking=no ${CHOST}  <<'ENDSSH'
	...
	pkg-config libnl-3-dev libnet-dev libcap-dev libbsd-dev python3-pip cmake
    	sudo apt  install make libncurses-dev git g++ python3 flex  bison bc  build-essential cmake   libedit-dev python zlib1g-dev rsync wget libelf-dev libssl-dev dwarves
	...
	echo 'export PS1="%(?:%{%}%{$fg[green]%}%n@%{$fg[red]%}%m ➜ :%{%}➜ ) %{$fg[cyan]%}%d%{$reset_color%}[cloud-lab] "'   >> ~/.zshrc
